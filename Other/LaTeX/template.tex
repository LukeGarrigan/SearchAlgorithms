...
\summary{
The goal of this project is to design, implement and compare algorithms for solving puzzles such as the sliding-tile and Rubik's Cube in the minimum number of moves. I will be implementing the iterative deepening A* algorithm, analysing and comparing it with the A* with Recursive Best First Search algorithm. I will also incoporate and test alternative algorithms such as Memory Bounded A*.
}
...
\item The option \texttt{final} provides a ``more official'' title page\footnote{You may want to show your report to a future employer.\label{refnote}}. It also creates a table of contents, and possibly list of tables and list of figures (see below). It does not include mark sheets at the end of the document\footref{refnote}. 
\end{itemize}
...
Note that \LaTeX{} truncates the final report to fifty-five pages as a limit of fifty pages is a requirement for this deliverable\footnote{A ten percent allowance above the required limit is made to comply with University regulations.}.
...
You do not need to use the command \verb/\maketitle/. Making the title is compulsory and is taken care of by the \texttt{cmpreport} class.

\section{Producing tables and figures} \label{sec2}

Tables are produced using a rewriting of the table environment that is specific to the report class. This new environment \verb/cmptable/ imposes some choices onto the user of the class to ensure a particular style in the report, but also provides new commands to facilitate the writing of quality tables. Tables~\ref{table1}, \ref{table2} and \ref{table3} show examples of tables produced using this environment. Details are provided in \citet{PCtut5}. 

\begin{cmptable}[h]{Homicides by race of the victim in 1993 in the USA\tnote{a}\label{table1}}
	\begin{tabular}{L{0.10}
			S[table-format=6]
			S[table-format=5]
			S[table-format=2.1, 
		...
		\toprule
		Race
		& \mcc(0.20){Population\tnote{b}}
		& \mcc(0.20){Homicides}
		& \mcc(0.10){Rate\tnote{c}} \\ \uhrule
	...
	\begin{tablenotes}
		...
		\note{$*p<.001$, two-tailed test. $**p<0.05$, two-tailed test.}
		\note{Adapted from \citet{Dewey}.}
	\end{tablenotes}
\end{cmptable}
\clearpage
\begin{cmptable}[h]
	...
	\begin{tabular}{L{0.05}L{0.35}SSS} 
		...
		\mcl[2]{\emph{Race and ethnicity:}} \\
		& African American (non-Hispanic) & 24.3 & 18.8 & 8.9 \\
		& Hispanic & 14.1 & 11.0 & 5.2 \\
		& Caucasian (non-Hispanic) & 6.0 & 4.0 & 3.0 \\ \addlinespace
		
		\mcl[2]{\emph{Gender:}} \\
		& Female & 25.1 & 23.7 & 4.8 \\
		& Male   & 11.1 & 3.8 & 9.3 \\ \midrule
		& Total  & 18.7 & 14.5 & 6.9 \\
		\bottomrule
	\end{tabular}
	\begin{tablenotes}
...
The Gantt chart in figure~\ref{pplan} is produced by the code in figure~\ref{codepplan}. To compile this code you need to include the command \verb|\usepackage{rotating}| in your preamble.  
...
\newganttchartelement{voidbar}{
voidbar/.style={ 
draw=black, top color=black!25, bottom color=black!23
}}
\begin{ganttchart}[x unit=0.45cm, vgrid, title label font=\scriptsize,
canvas/.style={draw=black, dotted}]{1}{34}
\gantttitle{Project schedule shown for e-vision week numbers
...
\gantttitlelist{1,...,10}{1} \gantttitle{EB}{4}
\gantttitlelist{11,...,14}{1}\\
...
\ganttlink{elem0}{elem2} \ganttlink{elem1}{elem2}
\ganttlink{elem2}{elem3} \ganttlink{elem5}{elem6}
...
\end{cmpfigure}
For more information on the \verb|pgfgantt| package see \citep{pgfgantt}. 
\clearpage
\bibliography{reportbib}

\appendix
\clearpage
\section{Code for the ``Cats'' figure} \label{ApB}
{\footnotesize
\begin{verbatim}
